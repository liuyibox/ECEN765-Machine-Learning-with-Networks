...
for i in range(len(data_train[0])):
    L1_norm[i] = np.sqrt(np.linalg.norm(data_train[:,i])/float(len(data_train)))
    if L1_norm[i] != 0:
...
for i in range(len(data_test[0])):
    L1_norm[i] = np.sqrt(np.linalg.norm(data_test[:,i])/float(len(data_test)))
    if L1_norm[i] != 0:
...
theta_train[0] = np.random.normal(0.0,1.0,35)
error_train = []
error_test = []
while(converged != True and train_iter < iter_criter):
    ...
    for i in range(len(data_test)):
        inner_product = np.inner(data_test[i], theta_train[train_iter - 1, :])
        result = int(np.sign(inner_product))
